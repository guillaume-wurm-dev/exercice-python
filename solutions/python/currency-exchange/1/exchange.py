...
def exchange_money(budget, exchange_rate) -> float:
    ...
    """
    value_exchange = budget / exchange_rate
    if value_exchange is not None :
...
def get_value_of_bills(denomination, number_of_bills) -> int:
    ...
    """
    value_total_of_bills = int(denomination) * number_of_bills
    if value_total_of_bills is not None :
        return value_total_of_bills
...
def get_number_of_bills(amount, denomination) -> int:
    ...
    if number_of_bills is not None :
        return number_of_bills


def get_leftover_of_bills(amount, denomination) -> bool:
...
def exchangeable_value(budget, exchange_rate, spread, denomination) -> int:
    ...
    taux_change = exchange_rate + (exchange_rate * spread / 100)
    value_of_currency = budget / taux_change
    number_bills = int(value_of_currency // denomination)  
